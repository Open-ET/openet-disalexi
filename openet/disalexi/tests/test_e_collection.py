...
def default_coll_args():
    return {
        ...
        'cloud_cover_max': 70,
        'model_args': {
            # The lai_source must be set to a collection ID for these tests
            # since calling the openet-landsat-lai module makes a getInfo call
...
def default_coll_obj(**kwargs):
    ...
    args.update(kwargs)
    return disalexi.Collection(**args)
